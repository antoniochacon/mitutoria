...
# -----------------------------------------------------------------
# Fuerza el reload de los archivos de static [FIN]
# -----------------------------------------------------------------
...
def informe_html(asignatura_id, tutoria_id, params={}):
    ...
    if request.method == 'POST':
        ...
        if informe_sql:
            # NOTE con esto me aseguro guardar el comentario cuando agrego calificaciones
...
def tutorias_html(params={}):
    ...
    params['show_tutorias_historial_all'] = params_old.get('show_tutorias_historial_all', False)
    ...
    if request.method == 'POST':
        ...
        if request.form['selector_button'] == 'selector_show_tutorias_historial_all':
            params['anchor'] = 'anchor_hist'
            params['show_tutorias_historial_all'] = True
            show_tutorias_historial_all = eval(request.form.get('show_tutorias_historial_all'))
            if show_tutorias_historial_all:
                g.settings_current_user.show_tutorias_historial_all = False
            else:
                g.settings_current_user.show_tutorias_historial_all = True
            session_sql.commit()
            return redirect(url_for('tutorias_html', params=dic_encode(params)))
        ...
        if request.form['selector_button'] == 'selector_tutoria_add':
            ...
            if not informe_preguntas():
                ...
                if tutoria_add_form.validate():
                    ...
                    # NOTE checkea si la fecha es anterior al dia actual
...
def settings_opciones_html(params={}):
    ...
    if request.method == 'POST':
        settings_edit_calendar_activado = False
        ...
        if request.form['selector_button'] == 'selector_settings_edit':
            ...
            if settings_edit_calendar:
                if not g.settings_current_user.calendar:
                    settings_edit_calendar_activado = True
                    g.settings_current_user.calendar = True
            else:
                if g.settings_current_user.calendar:
                    g.settings_current_user.calendar = False
            ...
            if settings_show_asignaturas_analisis:
            ...
            if settings_show_analisis_avanzado:
...
def settings_grupos_html(params={}):
    ...
    params['grupo_delete_confirmar'] = params_old.get('grupo_delete_confirmar', False)
    ...
    current_grupo_id = params['current_grupo_id']
    params['grupo_edit_link'] = params_old.get('grupo_edit_link', False)

    if params['grupo_delete_confirmar']:
        grupo_delete(current_grupo_id)
        flash_toast('Grupo elminado', 'success')
