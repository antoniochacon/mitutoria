...
def mantenimiento_calificaciones_nulas_clock():
    ...
    # calificaciones = session_sql.query(Calificacion).filter(Calificacion.created_at > current_date - datetime.timedelta(days=3),Calificacion.nota == 0, Calificacion.nombre == '').all()

    calificaciones = session_sql.query(Calificacion).filter(Calificacion.nota == 0, Calificacion.nombre == '').all()
        ...
        if asignaturas_id_lista:
            ...
            try:
                for asignatura_id in asignaturas_id_lista:
                    ...
                    # NOTE activar la primera linea para hacer pruebas y forzar un titulo distinto
                    ...
                    subject = 'Tutoría | %s | %s |  %s %s [24h]' % (grupo_activo_sql.nombre, alumno.nombre, tutoria_dia_semana, tutoria_dia_mes)
...
def translate_fecha(fecha):
    dic = {'Jan': 'Ene', 'Feb': 'Feb', 'Mar': 'Mar', 'Apr': 'Abr', 'May': 'May', 'Jun': 'Jun', 'Jul': 'Jul', 'Aug': 'Ago', 'Sep': 'Sep', 'Oct': 'Oct', 'Nov': 'Nov', 'Dec': 'Dic',
...
def evolucion_tutorias(alumno_id):
    ...
    tutorias_grupo_primera = session_sql.query(Tutoria).join(Alumno).join(Grupo).filter(Grupo.id == alumno.grupo_id, Tutoria.deleted == False).order_by('fecha').first()
    tutorias_grupo_ultima = session_sql.query(Tutoria).join(Alumno).join(Grupo).filter(Grupo.id == alumno.grupo_id, Tutoria.deleted == False).order_by(desc('fecha')).first()
    stats = {}
    ...
    evolucion_grupo_media_lista.append([arrow.get(tutorias_grupo_ultima.fecha + datetime.timedelta(weeks=4)).timestamp * 1000, round(evolucion_grupo_media, 1)])
